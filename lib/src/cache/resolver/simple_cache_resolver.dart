...
import 'cache_resolver_registry.dart';
...
final class SimpleCacheResolver implements CacheResolver, InitializingPod, PodFactoryAware, CacheResolverRegistry {
  ...
  FutureOr<Iterable<CacheStorage>> resolveCaches(Cacheable cacheable, MethodInvocation invocation) async {
    ...
    for (final resolver in _getResolvers()) {
      try {
        final resolved = await resolver.resolveCaches(cacheable, invocation);
    ...
    final cacheNames = ResourceUtils.resolveStorageNames(cacheable.cacheNames, invocation, Cacheable);
