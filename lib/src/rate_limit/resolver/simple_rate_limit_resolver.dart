...
import '../../util/resource_utils.dart';
...
import 'rate_limit_resolver_registry.dart';
...
final class SimpleRateLimitResolver implements RateLimitResolver, InitializingPod, PodFactoryAware, RateLimitResolverRegistry {
  ...
  FutureOr<Iterable<RateLimitStorage>> resolveStorages(RateLimit rateLimit, MethodInvocation invocation) async {
    ...
    for (final resolver in _getResolvers()) {
      try {
        final resolved = await resolver.resolveStorages(rateLimit, invocation);
    ...
    final storageNames = ResourceUtils.resolveStorageNames(rateLimit.storageNames, invocation, RateLimit);
